      ...
      { 
        currentQuestion === 0 ? (
          <form onSubmit={(e) => {e.preventDefault(); submitAnswer(currentQuestion, answerObject); handleQuestion('next'); } }>
            ...
            {answers.map((k, i) => (<input type="number" name={k.answerTag} placeholder={k.answerLabel} onChange={(e) => { const value = parseInt(e.target.value); handleAnswerInput(i, value); }} />)) }
            <button type="submit">Next</button>
          </form>
      ) : ( currentQuestion === 4 ? (
        <form onSubmit={(e) => {e.preventDefault(); submitAnswer(currentQuestion, selectedAnswer); }}>
        <label>{question}</label><br />
        { answers.map((k, i) => (<div className={styles.answerContainer}><label className={styles.label} for={k.answerTag}>{k.answerId}. {k.answerLabel}</label> <input type="checkbox" id={k.answerTag} name={k.answerTag} value={k.answerTag} onChange={(e) => {const value = parseInt(e.target.value); console.log(value); handleAnswerSelect(i, value);  }} /></div>)) }
        <button type="submit">Next</button>

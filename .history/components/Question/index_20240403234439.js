...
export default function Question({page = '1'}) {
  ...
  const answer = (page, obj = questions[`question_${page}`][`answer`]) => {
    ...
    var i = 0;
    for (let key in obj) {
      const changeKeyValue = () => { return key.replace(/_/gi, ` (`) +`)` };
      const newKey = key.match(/[_]/) ? changeKeyValue() : key;
      console.log(`change : `,  newKey);
      results += `<label for="question_${page}_${newKey}">${newKey}</label><input type="text" id="question_${page}_${newKey}" name="question_${page}_${newKey}" value="${newKey}"><br>`; }
    return results;
  }

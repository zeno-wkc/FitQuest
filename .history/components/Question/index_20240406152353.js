...
export default function Question() {
  ...
  const updateAnswer = (key, answerTag) => {
    console.log(key.answerId,answerTag);
    setAnswerObject(( key,prevAnswers) => ({
      ...prevAnswers,
      key: answerTag
    }))
  }
        ...
        <div className={styles.question}>{question}</div> 
          { 
            !showAns && activeQuestion == 0 ? 
              ...
              (<ul>{answers.map((k, i) => (<><li key={answers[i].answerId} id={answers[i].answerTag} onClick={() => {updateAnswer(k, answers[i].answerTag)}}>{answers[i].answerLabel}</li></>))}</ul>)
          }

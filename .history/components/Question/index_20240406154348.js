        ...
        <div className={styles.question}>{question}</div> 
          { 
            !showAns && activeQuestion == 0 ? 
              ...
              (<ul>{answers.map((k, i) => (<><li key={answers[i].answerId} id={answers[i].answerTag} onClick={() => {updateAnswer(activeQuestion, answers[i].answerTag)}}>{answers[i].answerLabel}</li></>))}</ul>)
          }

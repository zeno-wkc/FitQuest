    ...
    <div className={styles.quizContainer}>
      ...
      <div className={styles.questionContainer}>
        <div className={styles.question}>{question}</div> 
          ...
          { currentQuestion < questions.length - 1 && currentQuestion !== questions.length - 1 ? currentQuestion == 0 ? (<button onClick={() => handleQuestion('next') }>Next</button>) : (<><button onClick={() => handleQuestion('prev') }>Prev</button><button onClick={() => handleQuestion('next') }>Next</button></>) : (<div><h2>All questions answered!</h2><pre>{JSON.stringify(answerObject, null, 2)}</pre></div>) }
        </div>
    </div>
    </>
  );

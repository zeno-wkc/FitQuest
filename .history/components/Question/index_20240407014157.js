...
export default function Question() {
  ...
  return (
    ...
    <form onSubmit={(e) => {e.preventDefault(); submitAnswer(questions[1].id, selectedOption)}}>
      ...
      { questions[1].answers.map((k, i) => ( <> <label>{k.answerLabel}</label> <input type="radio" name={k.answerTag} value={k.answerTag} checked={selectedOption === k.answerTag} onChange={(e) => { setSelectedOption(e.target.value) }} /> </> )) }
      <button type="submit">Next</button>
    </form>

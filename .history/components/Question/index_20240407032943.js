  ...
  return (
    <div className={styles.quizContainer}>
      ...
      { 
        ...
        <label>{question}</label><br />
        { answers.map((k, i) => (<div className={styles.answerContainer}><label className={styles.label} for={k.answerTag}>{k.answerId}. {k.answerLabel}</label> <input type="checkbox" name={k.answerTag} value={k.answerTag}  onChange={(e) => { setSelectedOption(''); const value = parseInt(e.target.value);  handleAnswerSelect(i, value); }} /></div>)) }
        <button type="submit">Next</button>

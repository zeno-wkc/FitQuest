        ...
        <div>
          <form onSubmit={formSubmit}>
            ...
            { !showAns && currentQuestion == 0 ? 
              ...
              (<>{answers.map((k, i) => (<><input type="checkbox" key={answers[i].answerId} id={answers[i].answerTag} name={answers[i].answerLabel} value={answers[i].answerTag} /><label for={answers[i].answerTag}> {answers[i].answerLabel}</label></>))}</>)
            }
